...
var idProps = require('./parts/IdProps'),
    ...
    fetchPricetaskProps = require('./parts/FetchPriceTaskProps'),
...
function createGeneralTabGroups(
  ...
  var tempArr = [{ "name": "Default Flow", "value": "Default_Flow"}], tempValues = {};
  ...
  forEach(elementConfigurations[1].priceLists, function(option) {
    tempArrPrice.push({ "name": option.name, "value": option.name, id: option.id});
  });
  ...
  elementConfigurations.condOperator = [{"name":"Equals","value":"Equals"}];
  ...
  if (is(element, 'bpmn:Task') && bo.customElementName == SUITABILITY_FETCH_PRICE) {
    ...
    bo.fetchPriceData = elementConfigurations.FPPriceList;
    fetchPricetaskProps(group, element, bpmnFactory, canvas, translate, options);
  } else if (is(element, 'bpmn:Task') && bo.customElementName == SUITABILITY_PROCESS_PRICE) {
    taskProps(group, element, bpmnFactory, canvas, translate, options);
  } else if (is(element, 'bpmn:SequenceFlow')) {
    //taskProps(group, element, bpmnFactory, canvas, translate, options);
    sequenceFlowProps(group, element, bpmnFactory, canvas, translate, options);
  } 
  /* else {
