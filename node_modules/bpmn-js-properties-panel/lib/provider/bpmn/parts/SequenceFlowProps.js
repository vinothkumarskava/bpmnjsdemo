...
/*     var fpConditionKeyEl = domQueryAll("select[name='fpConditionKey']");
    if (fpConditionKeyEl && fpConditionKeyEl.length && !fpConditionKeyEl[0].selectedIndex) {
      var fpConditionOperatorEl = domQueryAll("select[name='fpConditionOperator']");
      if (fpConditionOperatorEl && fpConditionOperatorEl.length) {
        fpConditionOperatorEl[0].classList.add('bpp-hidden');
      }
    } */
  ...
  function canShowEl(key) {
    ...
    if (res[prop] && res[prop] == "Default_Flow") {
      return false;
    } else if (!res[prop] && selectedCondition) {
      return selectedCondition && selectedCondition[selectedCondition.selectedIndex].value == key && selectedCondition[selectedCondition.selectedIndex].value != "Default_Flow";
