...
module.exports = function(group, element, bpmnFactory, canvas, translate, additionalParams) {
  ...
  if (is(element, 'bpmn:Task') && bo.customElementName == SUITABILITY_FETCH_PRICE) {
    var options = { id:"ppPrice_list_operation", modelProperty: 'pricelist', label: translate('Price List ID'), inputArr: additionalParams.config.FPPriceList};
  ...
  if (is(element, 'bpmn:Task') && bo.customElementName == SUITABILITY_PROCESS_PRICE) {
    ...
    options = { id:"ppAdjustment_operation", modelProperty: 'adjustmentOperation', label: translate('Operation'), inputArr: additionalParams.config.PPAdjustmentOperation};
    group.entries = group.entries.concat(selectEntryFactory(element, options, translate));

    options = { id:"ppAdjustment_value", modelProperty: 'value', label: translate('Value (Percentage)') };
    ...
    options = { id:"ppRoundoff_operation", modelProperty: 'roundoffOperation', label: translate('Operation'), inputArr: additionalParams.config.PPRoundoffOperation};
    group.entries = group.entries.concat(selectEntryFactory(element, options, translate));

    // options = { id:"ppRoundoff_type", modelProperty: 'type', label: translate('Type'), inputArr: additionalParams.config.PPRoundoffType};
    // group.entries = group.entries.concat(selectEntryFactory(element, options, translate));

    options = { id:"ppRoundoff_type", modelProperty: 'type', label: translate('Type'), description: "Enter the rounding pattern here. Example: *.00, *.49, *.99, *.29"};
    group.entries = group.entries.concat(actionEntryFactory(element, options, translate));
  }
